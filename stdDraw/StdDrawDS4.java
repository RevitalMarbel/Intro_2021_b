 ...
 * @author Kevin Wayne
 */
public final class StdDrawDS4 implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
...
//	}
	...
	private static StdDrawDS4 std = new StdDrawDS4();
	...
	private static Image getImage(String filename) {
		...
		if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
			URL url = StdDrawDS4.class.getResource(filename);
			if (url != null)
				icon = new ImageIcon(url);
		}
	...
	public void actionPerformed(ActionEvent e) {
		FileDialog chooser = new FileDialog(StdDrawDS4.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
		chooser.setVisible(true);
		String filename = chooser.getFile();
		if (filename != null) {
			StdDrawDS4.save(chooser.getDirectory() + File.separator + chooser.getFile());
		}
	}
	...
	public void mousePressed(MouseEvent e) {
		synchronized (mouseLock) {
			mouseX = StdDrawDS4.userX(e.getX());
			mouseY = StdDrawDS4.userY(e.getY());
			isMousePressed = true;
		}
	}
	...
	public static double angleFrom(Point to ,Point from) {
		double Currangle= Math.atan2(from.y -to.y,from.x- to.x);
		Currangle=Math.toDegrees(Currangle);
		Currangle=(Currangle+360)%360;
		return Currangle;
	}
	public static void UnionByDist(int[] uF, Point[] points, double angle, Point p) {
		...
		for (int i = 0; i < uF.length; i++) {
			...
			double Currangle= angleFrom(p, points[i]);
	...
	public static void main(String[] args) {
		...
		StdDrawDS4.setPenColor(color[0]);
		...
		while (true) {
			...
			p.x=50;
			p.y=50;
			UnionByDist(unionFind, points, dist,p);
			for (int j = 0; j < unionFind.length; j++) {
				StdDrawDS4.setPenColor(color[Find(j, unionFind)]);
				StdDrawDS4.filledCircle(points[j].getX() / 100, points[j].getY() / 100, 0.007);
			}
			dist+=1;

		//	if (!StdDraw.isKeyPressed(KeyEvent.VK_SPACE)) {
				while (!StdDrawDS4.isKeyPressed(KeyEvent.VK_SPACE)) {
					StdDrawDS4.pause(100);
