 ...
 * segment from (0.5, –&infin;) to (0.5, &infin;) may not be visible even in the
...
//	}
	...
	private static final double DEFAULT_XMIN = -0.1;
	private static final double DEFAULT_XMAX = 1.1;
	private static final double DEFAULT_YMIN = -0.1;
	private static final double DEFAULT_YMAX = 1.1;
	...
	public void actionPerformed(ActionEvent e) {
		FileDialog chooser = new FileDialog(StdDraw2.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
		chooser.setVisible(true);
		String filename = chooser.getFile();
		if (filename != null) {
			StdDraw2.save(chooser.getDirectory() + File.separator + chooser.getFile());
		}
	}
	...
	public static int Find(int p, int[] arr) {
		while (arr[p] != p)
			p = arr[p];
		return p;
	}

	public static void Union(int ind1, int ind2, int[] arr) {
		int temp2 = Find(ind2, arr);
		int temp1 = Find(ind1, arr);
		// System.out.println(temp1+" "+ temp2);
		arr[temp1] = temp2;
	}

	// return arandom point array of size $size$
	public static PointMy[] generateRandomArray(int size) {
		PointMy[] points = new PointMy[size];
		for (int i = 0; i < size; i++) {
			int x = (int) (Math.random() * 100);
			int y = (int) (Math.random() * 100);
			PointMy p = new PointMy(new Point(x, y), i);
			points[i] = p;
		}
		return points;
	}
	...
	public static Color[] generateColorrray(int size) {
		...
		for (int i = 0; i < size; i++) {
			Color c = new Color(b % 255, (b + 50) % 255, (b + 100) % 255);
			color[i] = c;
			b += jump;
		}
		return color;
	...
	public static ArrayList<Distance> generateDistArray(Point[] p) {
		ArrayList<Distance> dist = new ArrayList<Distance>();
		for (int i = 0; i < p.length; i++) {
			for (int j = i + 1; j < p.length; j++) {
				Distance d = new Distance(p[i], p[j]);
				dist.add(d);
			}
		}
		return dist;
	...
	public static void sortBydistance(ArrayList<Distance> dist) {
		Collections.sort(dist);
	}

	public static void UnionByDist(int[] uF, Point[] points, double dist) {
		for (int i = 0; i < uF.length; i++) {
			for (int j = i + 1; j < uF.length; j++) {
				if (points[i].distance(points[j]) < dist) {
					Union(i, j, uF);
				}
			}
		}
	...
	public static void UnionByPolarDist(int[] uF, Point[] points, double angle, Point p) {
		ArrayList<Integer>[] angleArr = new ArrayList[(int)(360/angle)+1];
		for (int i = 0; i < angleArr.length; i++) {
			angleArr[i]= new ArrayList<Integer>();
		}
		
		for (int i = 0; i < uF.length; i++) {
			//for (int j = i + 1; j < uF.length; j++) {
			double Currangle= Math.atan2(p.y -points[i].y,p.x- points[i].x);
			Currangle=Math.toDegrees(Currangle);
			Currangle=(Currangle+360)%360;
			angleArr[(int)(Currangle/angle)].add(i);
					//Union(i, j, uF);
				
			}
		for (int j = 0; j < angleArr.length; j++) {
			for (int j2 = 1; j2 < angleArr[j].size(); j2++) {
				Union(angleArr[j].get(0), angleArr[j].get(j2), uF);
			}
		}
	...
	public static void main(String[] args) {
//		StdDraw2.square(0.2, 0.8, 0.1);
//		StdDraw2.filledSquare(0.8, 0.8, 0.2);
		// Point[] points= {new Point(2,2),new Point(5,1),new Point(2,2),new
		// Point(4,4),new Point(7,4),new Point(2,4),new Point(9,4),new Point(5,9),new
		// Point(4,3) };
		// Color[] color= {StdDraw2.BOOK_BLUE,StdDraw2.BLACK,
		// StdDraw2.BOOK_RED,StdDraw2.CYAN,StdDraw2.GREEN,StdDraw2.DARK_GRAY,StdDraw2.MAGENTA,StdDraw2.PINK,StdDraw2.RED};
		Point[] points = generateRandomArray(600);
		Color[] color = generateColorrray(600);
		//ArrayList<Distance> dists = generateDistArray(points);
		//sortBydistance(dists);

		StdDraw2.setPenColor(color[0]);
...
//		System.out.println(Find(1,unionFind));
//		Union(1, 2, unionFind);
...
//		Union(5,2 , unionFind);
		...
		while (true) {
			...
			p.x=50;
			p.y=50;
			UnionByDist(unionFind, points, dist);
			for (int j = 0; j < unionFind.length; j++) {
				StdDraw2.setPenColor(color[Find(j, unionFind)]);
				StdDraw2.filledCircle(points[j].getX() / 100, points[j].getY() / 100, 0.007);
			}
			dist+=1;

		//	if (!StdDraw.isKeyPressed(KeyEvent.VK_SPACE)) {
				while (!StdDraw2.isKeyPressed(KeyEvent.VK_SPACE)) {
					StdDraw2.pause(100);
